...
- [The Rust platform dependencies have been updated][48]: dirs, sysinfo, windows
  ...
  other platforms).
- [The Rust derive-macro dependencies have been updated][47]: syn 3, derive_more
  2 and strum 0.28. No behaviour changes; the parser produces the same AST and
  the same Java bindings as before.
- [The build tooling no longer depends on the unmaintained `serde_yaml`][45]. No
  behaviour changes; the generated CI workflows are byte-identical.
...
- [Fixed a startup race in the local library repository server][41]. It
  announced itself as ready before it began accepting connections, so a client
  connecting in that window was refused.
...
[41]: https://github.com/jdunkerley/enso/pull/41
