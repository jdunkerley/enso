...
import {
  useComponentBrowserInput,
  type ComponentBrowserMode,
...
`('Reading literal from $inputContent', ({ inputContent, expectedLiteral }) => {
  const input = useComponentBrowserInput(() => false, mockGraphDb(), new SuggestionDb(), ref())
...
`(
  ...
  ({ inputContent, source, expectedCode }) => {
    ...
    const input = useComponentBrowserInput(() => false, db, new SuggestionDb(), ref())
...
`('Applying suggestion with possible type casting', ({ suggestionId, expectedCode }) => {
  ...
  const input = useComponentBrowserInput(() => false, db, suggestionDb, ref())
...
describe('useComponentBrowserInput mode selection', () => {
  ...
  test('newNode defaults to aiPrompt when AI is available', () => {
    const input = useComponentBrowserInput(() => true, mockGraphDb(), new SuggestionDb(), ref())
    input.reset({ type: 'newNode' })
  ...
  test('newNode reopens in the mode the user last picked', () => {
    const lastPicked = ref<ComponentBrowserMode>()
    ...
    const reopened = useComponentBrowserInput(
      () => true,
      mockGraphDb(),
      new SuggestionDb(),
      lastPicked,
    )
    reopened.reset({ type: 'newNode' })
    expect(reopened.selectedMode).toBe('codeEditing')
  })

  test('a remembered aiPrompt degrades to componentBrowsing while AI is unavailable', () => {
    const lastPicked = ref<ComponentBrowserMode>('aiPrompt')
    const input = useComponentBrowserInput(
      () => false,
      mockGraphDb(),
      new SuggestionDb(),
  ...
  test('accepting raw input as code is not remembered as a mode pick', () => {
    const lastPicked = ref<ComponentBrowserMode>()
    const input = useComponentBrowserInput(
      () => true,
      mockGraphDb(),
  ...
  test('applySuggestion switches to codeEditing and exposes the applied entry', () => {
    const input = useComponentBrowserInput(() => true, mockGraphDb(), mockSuggestionDb(), ref())
    input.reset({ type: 'newNode' })
    expect(input.selectedMode).toBe('aiPrompt')
